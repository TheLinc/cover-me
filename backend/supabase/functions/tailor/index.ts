...
function stripScores(r: Record<string, unknown>): Record<string, unknown> {
  const { atsScore: _s, atsGaps: _g, ...rest } = r
  return rest
}

function buildPrompt(job: JobPayload, parsed: ParsedResume, compact: boolean, supplemental?: string, trim = false, includeSummary = true, previous?: Record<string, unknown>): string {
...
Deno.serve(async (req) => {
  ...
  // (Haiku, verbatim extraction) and cache it encrypted for future tailors.
  // resume/index.ts clears the cache whenever the resume text is replaced.
  let structured: ParsedResume | null = null
  if (resumeRow.structured_encrypted) {
    try {
      const decoded = JSON.parse(await decrypt(resumeRow.structured_encrypted))
      if (isValidParsedResume(decoded)) structured = decoded
    } catch {
      structured = null
    }
  }
  if (!structured) {
    ...
    try {
      structured = await parseResumeStructure(resumeText, ANTHROPIC_API_KEY)
    } catch {
      await refund()
      return json({ error: 'Could not process your resume. Please try again.' }, 502)
    }
    // Cache best-effort — a failed write just means we re-parse next time.
    try {
      const structured_encrypted = await encrypt(JSON.stringify(structured))
      await supabase.from('resumes').update({ structured_encrypted }).eq('user_id', userId)
    } catch {
      console.error('Failed to cache structured resume')
    }
  }

  const prompt = buildPrompt(job, structured, compact, supplemental, trim, includeSummary, previous)
  ...
  if (DEBUG) {
    ...
    console.log('[CoverMe debug] structured resume (input to model):\n', JSON.stringify(structured))
  ...
  const structuredResume = structured

  // Reads Anthropic's SSE stream, forwarding each text fragment to onDelta and
  // returning the full accumulated text.
  const readUpstream = async (onDelta?: (text: string) => void): Promise<string> => {
  ...
  // Parse the delta, merge into the structured resume, attach the code-computed
  // score, and persist for Pro users. Throws on unusable model output.
  const assemble = async (raw: string): Promise<Record<string, unknown>> => {
  ...
  // Legacy clients (extension versions before the streaming update) expect a
  // plain JSON body; new clients opt into NDJSON via the Accept header. Chrome
  // Web Store updates roll out over days, so both must keep working.
  const wantsStream = (req.headers.get('accept') ?? '').includes('application/x-ndjson')
  if (!wantsStream) {
    try {
      const resume = await assemble(await readUpstream())
      return json({ resume })
    } catch (err) {
      console.error('Tailor error:', err instanceof Error ? err.message : err)
      await refund()
      return json({ error: 'AI generation failed. Please try again.' }, 502)
    }
  }

  // Stream NDJSON to the extension: a start event (role count for the progress
  // label), one delta event per text fragment, then a final done event carrying
  // the fully merged + scored resume. Streaming from the first token keeps the
  // connection alive on slow generations and lets the popup show real progress.
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        send({ type: 'done', resume: await assemble(raw) })
      } catch (err) {
        console.error('Tailor stream error:', err instanceof Error ? err.message : err)
        await refund()

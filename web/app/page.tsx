    ...
    >
      <div className="container">
        <div className="flex items-center gap-8">
          <a href="/" className="flex items-center gap-[9px] text-[15px] font-bold text-foreground tracking-[-0.3px] shrink-0">
            <Image src="/logo.png" width={26} height={26} alt="Cover Me" />
            <span>Cover Me</span>
          </a>
          <div className="flex gap-7 flex-1 max-md:hidden">
            <a href="#how-it-works" className="text-[13px] font-medium text-dim hover:text-foreground transition-colors tracking-[0.01em]">How it works</a>
            <a href="#features"     className="text-[13px] font-medium text-dim hover:text-foreground transition-colors tracking-[0.01em]">Features</a>
            <a href="#pricing"      className="text-[13px] font-medium text-dim hover:text-foreground transition-colors tracking-[0.01em]">Pricing</a>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button asChild size="sm">
              <a href="https://chrome.google.com/webstore" target="_blank" rel="noreferrer">Install free</a>
            </Button>
          </div>
        </div>
      </div>
...
function WorkflowDemo() {
  ...
  return (
    ...
    >
      ...
      {/* Browser content */}
      <div className="flex h-[390px] bg-surface relative overflow-hidden max-[768px]:h-80 max-[540px]:h-[260px]">

        {/* Job listing */}
        <div className="flex-1 px-9 py-7 overflow-hidden max-[1100px]:px-6 max-[768px]:px-5">
          <div className="flex items-center gap-3.5 pb-5 border-b border-border mb-5">
            ...
            <div>
              <p className="text-[15px] font-bold text-foreground tracking-[-0.3px]">Senior Frontend Engineer</p>
              <p className="text-[11.5px] text-dim mt-[3px]">Stripe · San Francisco, CA · Remote · $180K–$240K</p>
            </div>
          ...
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.08em] text-dim mb-2 mt-0">About the role</p>
            <p className="text-xs text-muted-foreground leading-[1.75] mb-2 max-[768px]:text-[11px]">
              We&apos;re looking for a frontend engineer to help build the tools that power the internet economy. You&apos;ll work on our developer dashboard, improve our React component library, and ship features used by millions of businesses worldwide.
            </p>
            <p className="text-xs text-muted-foreground leading-[1.75] mb-2 max-[768px]:text-[11px]">
              You have strong opinions about performance, accessibility, and developer experience. You move fast and care about quality.
            </p>
            <p className="text-[10px] font-bold uppercase tracking-[0.08em] text-dim mb-2 mt-3.5">Qualifications</p>
            <p className="text-xs text-muted-foreground leading-[1.75] max-[768px]:text-[11px]">
              5+ years of React experience, strong TypeScript, experience with large-scale web applications.
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div className="h-[204px] relative overflow-hidden">
            ...
            >
              {[[91, 84, 67], [88, 96, 58], [76, 42]].map((widths, gi) => (
                <div key={gi} className="flex flex-col gap-1.5">
                  {widths.map((w, li) => (
                    <div key={li} className="skeleton-line h-[9px]" style={{ width: `${w}%` }} />
                  ))}
                </div>
              ))}
            </div>
            ...
            >
              {DEMO_PARAS.map((p, i) => (
                <p key={i} className="text-[10.5px] leading-[1.75] text-foreground">{p}</p>
              ))}
            </div>
          </div>
          ...
          <div className="px-3 py-[9px] border-t border-border flex gap-1.5">
            ...
            <button className="bg-elevated text-muted-foreground border border-border rounded-[4px] py-[7px] px-2.5 text-[11px] font-semibold cursor-default">PDF</button>
            <button className="bg-elevated text-muted-foreground border border-border rounded-[4px] py-[7px] px-2.5 text-[11px] font-semibold cursor-default">Edit</button>
          </div>
        </div>
      </div>

      {/* Caption */}
      <div className="h-9 bg-elevated border-t border-border flex items-center justify-center text-[11px] text-dim tracking-[0.01em]">
        {phase === 0 && <span key="0" style={{ animation: 'fadeIn 0.4s ease both' }}>Open a job posting on any board&hellip;</span>}
        {phase === 1 && <span key="1" style={{ animation: 'fadeIn 0.4s ease both' }}>Reading the posting and your resume&hellip;</span>}
        {phase === 2 && <span key="2" style={{ animation: 'fadeIn 0.4s ease both' }}>Done in under 5 seconds &mdash; edit, copy, or download as PDF.</span>}
      </div>
    </div>
  );
}
    ...
    <section className="pt-[160px] pb-20 relative overflow-hidden">
      ...
      <div className="container">
        ...
        >
          ...
          <p className="flex items-center gap-2.5 text-[11px] text-dim tracking-[0.01em]">
            Free forever
...
function WorksOn() {
  const boards = ["LinkedIn", "Indeed", "Greenhouse", "Lever", "Workday", "Ashby", "Any job board"];
  return (
    <div className="py-[18px] border-t border-b border-border">
      <div className="container">
        <div className="flex items-center gap-[18px] flex-wrap">
          <span className="text-[10px] font-bold uppercase tracking-[0.09em] text-dim whitespace-nowrap">Works on</span>
          <div className="flex items-center gap-2.5 flex-wrap">
            {boards.map((b, i) => (
              <span key={b} className="flex items-center gap-2.5">
                <span className="text-[12.5px] font-medium text-muted-foreground">{b}</span>
                {i < boards.length - 1 && <span className="inline-block w-[3px] h-[3px] rounded-full bg-border" />}
              </span>
            ))}
          </div>
        </div>
...
function HowItWorks() {
  ...
  return (
    <section id="how-it-works" className="py-[120px] max-[768px]:py-20" ref={ref}>
      <div className="container">
        ...
        <div className="border-t border-border">
          {STEPS.map((s, i) => (
            <div
              key={s.n}
              className={cn(
                "grid grid-cols-[52px_1fr] [gap:0_48px] py-11 border-b border-border items-start opacity-0 translate-y-4 transition-[opacity,transform] duration-500 max-[900px]:grid-cols-[40px_1fr] max-[900px]:[gap:0_28px] max-[768px]:grid-cols-[36px_1fr] max-[768px]:[gap:0_20px] max-[768px]:py-8",
                visible && "opacity-100 translate-y-0"
              )}
              style={{ transitionDelay: `${i * 0.12}s` }}
            >
              <span className="text-[10px] font-bold text-brand tracking-[0.1em] uppercase pt-[5px]">{s.n}</span>
              <div className="flex flex-col gap-3">
                <h3 className="text-[19px] font-bold text-foreground tracking-[-0.4px] leading-[1.25]">{s.title}</h3>
                <p className="text-[14px] text-muted-foreground leading-[1.8] max-w-[660px]">{s.body}</p>
              </div>
            </div>
          ))}
        </div>
...
const FEATURES: { title: string; body: string; accent?: boolean; span: string; pad?: string }[] = [
  {
      ...
      <div className="container">
        ...
        <div className="grid grid-cols-6 max-[1100px]:grid-cols-4 max-[900px]:grid-cols-2 max-[768px]:grid-cols-1 gap-px bg-border border border-border rounded-[10px] overflow-hidden">
          {FEATURES.map((f, i) => (
            ...
            >
              <h3 className="text-[14px] font-bold text-foreground tracking-[-0.2px] leading-[1.3]">{f.title}</h3>
              <p className="text-[13px] text-muted-foreground leading-[1.75]">{f.body}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="container">
        <div className="grid grid-cols-[1fr_180px] gap-20 items-center max-[900px]:grid-cols-1 max-[900px]:gap-10">
          <div className="flex flex-col gap-[22px]">
            <span className="text-[10px] font-bold tracking-[0.12em] uppercase text-brand">Open source</span>
            <h2 className="text-[clamp(28px,3.5vw,44px)] font-extrabold tracking-[-1.5px] leading-none text-foreground">
              Built in public.<br />Auditable by anyone.
            </h2>
            ...
            <div className="flex items-center gap-5 flex-wrap">
              <Button asChild variant="outline">
                <a href="https://github.com/TheLinc/cover-me" target="_blank" rel="noreferrer">
                  View on GitHub
                  <ArrowUpRightIcon size={11} />
                </a>
              </Button>
              <span className="text-xs text-dim">MIT License · No telemetry · No ads</span>
            </div>
          </div>
          <div className="flex items-center justify-center max-[900px]:hidden">
            <svg viewBox="0 0 98 96" fill="currentColor" className="w-[100px] h-[100px] text-dim opacity-25">
...
function Pricing() {
  return (
    <section id="pricing" className="py-[120px] max-[768px]:py-20">
      <div className="container">
        <div className="flex flex-col gap-2.5 mb-[52px]">
          <span className="text-[10px] font-bold tracking-[0.12em] uppercase text-brand">Pricing</span>
          <h2 className="text-[clamp(30px,3.8vw,48px)] font-extrabold tracking-[-1.5px] leading-none text-foreground max-w-[600px]">
            Start free. Upgrade when you&apos;re ready.
          </h2>
          <p className="text-[15px] text-muted-foreground leading-[1.65] max-w-[480px] mt-1">
            No contracts. Cancel any time. Or use your own API key for free, forever.
          </p>
        </div>
        <div className="grid grid-cols-2 max-[900px]:grid-cols-1 max-[900px]:max-w-[480px] gap-px max-w-[780px] bg-border border border-border rounded-[10px] overflow-hidden">
          ...
          <div className="bg-surface px-9 py-10 flex flex-col relative">
            <p className="text-[10px] font-bold uppercase tracking-[0.1em] text-dim mb-3">Free</p>
            <div className="flex items-baseline gap-1 mb-3">
              <span className="text-[52px] font-extrabold tracking-[-2.5px] leading-none text-foreground">$0</span>
              <span className="text-[14px] text-dim font-medium">/forever</span>
            </div>
            ...
            <ul className="list-none flex flex-col gap-2.5 flex-1 mb-7">
              {FREE_FEATURES.map((f) => (
                <li key={f} className="flex items-start gap-2.5 text-[13px] text-muted-foreground leading-[1.5]">
                  <CheckIcon size={13} className="shrink-0 text-dim mt-[2px]" />
                  {f}
                </li>
              ))}
            </ul>
            <Button asChild variant="outline" className="w-full justify-center py-3 text-[13px]">
              <a href="https://chrome.google.com/webstore" target="_blank" rel="noreferrer">Install free</a>
            </Button>
          </div>
          ...
          <div className="bg-gradient-to-br from-surface to-[rgba(99,102,241,0.04)] px-9 py-10 flex flex-col relative">
            ...
            <p className="text-[10px] font-bold uppercase tracking-[0.1em] text-dim mb-3">Pro</p>
            <div className="flex items-baseline gap-1 mb-3">
              <span className="text-[52px] font-extrabold tracking-[-2.5px] leading-none text-brand-light">$4</span>
            ...
            <ul className="list-none flex flex-col gap-2.5 flex-1 mb-7">
              {PRO_FEATURES.map((f) => (
                <li key={f} className="flex items-start gap-2.5 text-[13px] text-muted-foreground leading-[1.5]">
                  <CheckIcon size={13} className="shrink-0 text-brand-light mt-[2px]" />
                  {f}
                </li>
              ))}
            </ul>
      ...
      <div className="container">
        <div className="flex gap-20 pt-14 pb-10 items-start max-[768px]:flex-col max-[768px]:gap-9">
          <div className="flex flex-col gap-3 flex-1">
            <a href="/" className="flex items-center gap-[9px] text-[15px] font-bold text-foreground tracking-[-0.3px]">
          ...
          <div className="flex gap-[60px] max-[768px]:gap-10">
            <div className="flex flex-col gap-2.5">
              <span className="text-[10px] font-bold uppercase tracking-[0.09em] text-dim mb-0.5">Product</span>
              <a href="#how-it-works" className="text-[13px] text-dim hover:text-foreground transition-colors">How it works</a>
              <a href="#features"     className="text-[13px] text-dim hover:text-foreground transition-colors">Features</a>
              <a href="#pricing"      className="text-[13px] text-dim hover:text-foreground transition-colors">Pricing</a>
              <a href="https://chrome.google.com/webstore" target="_blank" rel="noreferrer" className="text-[13px] text-dim hover:text-foreground transition-colors">Install</a>
            </div>
            <div className="flex flex-col gap-2.5">
              <span className="text-[10px] font-bold uppercase tracking-[0.09em] text-dim mb-0.5">Open source</span>
              <a href="https://github.com/TheLinc/cover-me" target="_blank" rel="noreferrer" className="text-[13px] text-dim hover:text-foreground transition-colors">GitHub</a>
              <a href="/privacy" className="text-[13px] text-dim hover:text-foreground transition-colors">Privacy</a>
              <a href="/terms"   className="text-[13px] text-dim hover:text-foreground transition-colors">Terms</a>
            </div>
            <div className="flex flex-col gap-2.5">
              <span className="text-[10px] font-bold uppercase tracking-[0.09em] text-dim mb-0.5">Account</span>
              <a href="/auth"         className="text-[13px] text-dim hover:text-foreground transition-colors">Sign in</a>
              <a href="/auth?plan=pro" className="text-[13px] text-dim hover:text-foreground transition-colors">Upgrade to Pro</a>
              <a href="/dashboard"    className="text-[13px] text-dim hover:text-foreground transition-colors">Dashboard</a>
            </div>
          </div>
        </div>
        <div className="flex justify-between items-center pt-6 border-t border-border text-[11px] text-dim">
